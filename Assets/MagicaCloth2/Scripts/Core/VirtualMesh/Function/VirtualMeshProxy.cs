...
namespace MagicaCloth2
{
    ...
    {
        /// <summary>
        /// メッシュをプロキシメッシュに変換する（スレッド可） 将网格转换为代理网格（可线程）
        /// プロキシメッシュは頂点法線接線を接続するトライアングルから求めるようになる 代理网格是从连接顶点法线切线的三角中求出的
        /// またマッピング用の頂点ごとのバインドポーズも保持する 还保留映射顶点的绑定姿势
        /// </summary>
            ...
            )
        {
            try
            {
                ...
                // 頂点に接続する頂点リストを求める 求与顶点连接的顶点列表 (顶点到顶点)
                // エッジリストを作成する 创建边列表
                ...
                if (TriangleCount > 0)
                {
                    ...
                    // トライアングルの向きをできる限り揃える 尽量使三角的方向一致
        ...
        void OptimizeTriangleDirection(NativeArray<float3> triangleNormals, float sameSurfaceAngle)
        {
            ...
            while (startIndex < TriangleCount)
            {
                ...
                while (triQueue.Count > 0)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        foreach (var data in edgeToTriangles.GetValuesForKey(edge))
                        {
                            ...
                            // 面の法線が一定方向を向くように調整する
                            // 调整面的法线朝向一定方向，  相当于两个三角形夹角大于90度
                            if (math.dot(n, n2) < 0.0f)
                            {
                                // フリップ
                                // 翻转 //相当于三角形正反面翻转，也就是法线翻转
                            ...
                            // 计算相邻三角线的法线是打开还是关闭  //相当于两个三角形，面的夹角大于90度是打开的，小于90度是关闭的（最大180）
                ...
                // 閉じているトライアングルのほうが多い場合はレイヤー全体の法線をフリップさせる 如果关闭的三角更多，则翻转整个层的法线
        ...
        //=========================================================================================
        /// <summary>
        /// セレクションデータ属性をプロキシメッシュに反映させる（スレッド可） 将选择数据属性反映到代理网格中（可线程）
        /// </summary>
