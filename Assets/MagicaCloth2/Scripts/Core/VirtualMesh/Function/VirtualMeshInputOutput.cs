...
namespace MagicaCloth2
{
    ...
    {
        /// <summary>
        /// レンダー情報からインポートする（スレッド可） 从渲染信息导入（可线程）
        /// </summary>
        ...
        public void ImportFrom(RenderSetupData rsetup)
        {
            try
            {
                // ★基本的に一連の作業が完了するまでキャンセルはさせない（安全性のため） 基本上在完成一系列作业之前不会取消（为了安全性）
                // 基本上在完成一系列作业之前不会取消（为了安全性）
                if (rsetup == null)
                {
                ...
                // ========== Transform =========
                // セットアップ情報からトランスフォームを登録する 从设置信息注册变换
                // 从设置信息登录变换
                ...
                // ========== ここからMesh/Boneで分岐 从这里开始用网格/布尔分支 ==========
                ...
                switch (rsetup.setupType)
                {
                    ...
                    case RenderSetupData.SetupType.BoneSpring:
                        if (TriangleCount > 0)
                        {
                            // ボーンタイプでトライアングルを含む場合 骨骼类型中包含三角的情况
                            // 骨骼类型中包含三角的情况
        ...
        void ImportBoneType(RenderSetupData rsetup, int[] transformIndices)
        {
            ...
            // Transformの情報をクロスローカル空間に変換し頂点情報に割り当てる 将变换的信息转换为交叉局部空间并指定给顶点信息
            // およびバインドポーズの算出 以及绑定姿势的计算
            ...
            // 参照インデックスに連番を振る 对引用索引编号
            JobUtility.SerialNumberRun(referenceIndices.GetNativeArray(), vcnt);

            // Line/Triangleの形成
            // ★ここは数も少なくあまりBurstの恩恵を受けられないので普通にC#で構成する
            // 这里的数量也很少，不能得到Burst的好处，所以一般由C#构成
            if (rsetup.boneConnectionMode == RenderSetupData.BoneConnectionMode.Line)
            {
                ...
                {
                    // 親に接続させる 连接到父代
                    int pindex = rsetup.GetParentTransformIndex(i, true); // センタートランスフォームは除外 排除中心变换
                    if (pindex >= 0)
                    {
                ...
                {
                    // スプリングではコリジョン無効で初期化 弹簧的碰撞无效初始化
                    attributes.Fill(0, vcnt, VertexAttribute.DisableCollision);

                    // コリジョンとして指定されたボーンのみ衝突判定を有効化する 仅将指定为碰撞的骨骼启用碰撞判定
            ...
            {
                // Mesh接続 网格连接
                // トランスフォームIDからインデックスへの辞書を作成 创建从变换ID到索引的词典
                ...
                // 頂点ごとの接続情報の作成 创建每个顶点的连接信息
                var linkList = new FixedList128Bytes<int>[vcnt]; //每一个粒子直接连接的粒子（单单从骨骼树的结构上）
                var vertexLvList = new int[vcnt]; // 存储每个粒子的深度/级别
                var vertexRootIndex = new int[vcnt]; // 存储每个粒子的根
                var lvIndexList = new List<FixedList512Bytes<int>>(); // レベルごとのリスト 按级别列出， 根据级别(深度)，存储粒子
                var mainEdgeSet = new HashSet<uint>(); // メインエッジ 主边 相当于是骨骼树
                ...
                // 次に同レベルの横を接続する 然后连接同一级别的横向
                // 主要往linkList增加连接相同深度的邻居骨骼
                ...
                for (int i = 0; i < vcnt; i++)
                {
                    ...
                    foreach (var vindex in lvList)
                    {
                        ...
                        // 非ループならば始点と終点のルートラインは接続しない 如果是非循环，则不连接起点和终点的根线
                        ...
                        // 順次接続なら自身の前後のルートラインのみ接続 依次连接的话只连接自己前后的根线
                    ...
                    if (firstIndex >= 0)
                    {
                        ...
                        // 次に最初に接続した距離の少し大きめの範囲にあるすべての頂点をつなげる 接着连接最初连接距离稍大范围内的所有顶点
                        // ただし順次接続の場合は距離は無視する 但是依次连接时忽略距离
                ...
                var triangleEdgeSet = new HashSet<int2>(); //每个元素是三角形的一个边
                var triangleSet = new HashSet<int3>(); // 每个元素是三角形的三个顶点索引
                for (int i = 0; i < vcnt; i++)
                {
                    var link = linkList[i];
                    if (link.Length == 0)
                ...
                // ライン登録  行注册
                // トライアングルに利用されなかったエッジのみ登録 仅注册未用于三角洲的边
        ...
        public void SetCustomSkinningBones(TransformRecord clothTransformRecord, List<TransformRecord> bones)
        {
            ...
            for (int i = 0; i < bones.Count; i++)
            {
                ...
                if (rd.IsValid())
                {
                    ...
                    int tindex = transformData.AddTransform(rd, checkDuplicate: false); // 重複ありで最後に追加する 重复，最后添加
