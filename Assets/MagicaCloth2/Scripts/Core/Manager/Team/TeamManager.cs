...
namespace MagicaCloth2
{
    public class TeamManager : IManager, IValid
    {
        ...
        /// チームフラグ(64bit)
        /// </summary>
        public const int Flag_Valid = 0; // データの有効性 数据有效性
        public const int Flag_Enable = 1; // 動作状態 工作状态
        public const int Flag_Reset = 2; // 姿勢リセット 姿势复位
        public const int Flag_TimeReset = 3; // 時間リセット 时间复位
        public const int Flag_Suspend = 4; // 一時停止 暂停
        public const int Flag_Running = 5; // 今回のフレームでシミュレーションが実行されたかどうか 在这次框架中是否执行了模拟
        ...
        public const int Flag_StepRunning = 7; // ステップ実行中 步骤执行中
        public const int Flag_Exit = 8; // 存在消滅時  存在消失时
        public const int Flag_KeepTeleport = 9; // 姿勢保持テレポート 姿势保持电话
        public const int Flag_InertiaShift = 10; // 慣性全体シフト  惯性整体位移
        public const int Flag_CullingInvisible = 11; // カリングによる非表示状態 标记隐藏状态
        public const int Flag_CullingKeep = 12; // カリング時に姿勢を保つ  卡环时保持姿势
        public const int Flag_Spring = 13; // Spring利用  Spring利用
        public const int Flag_SkipWriting = 14; // 書き込み停止（ストップモーション用）  写入停止（停止动作用）
        public const int Flag_Anchor = 15; // Inertia anchorを利用中   正在使用Inertia anchor
        public const int Flag_AnchorReset = 16; // Inertia anchorの座標リセット   重置Inertia anchor坐标
        public const int Flag_NegativeScale = 17; // マイナススケールの有無    有无负标度
        public const int Flag_NegativeScaleTeleport = 18; // マイナススケールによるテレポート  使用负比例传输

        // 以下セルフコリジョン /以下为自碰撞
        // !これ以降の順番を変えないこと !不要改变以后的顺序
        public const int Flag_Self_PointPrimitive = 32; // PointPrimitive+Sortを保持し更新する 保留并更新PointPrimitive+Sort
        public const int Flag_Self_EdgePrimitive = 33; // EdgePrimitive+Sortを保持し更新する 保留并更新EdgePrimitive+Sort
        public const int Flag_Self_TrianglePrimitive = 34; // TrianglePrimitive+Sortを保持し更新する 保留并更新TrianglePrimitive+Sort
        ...
        internal int AddTeam(ClothProcess cprocess, ClothParameters clothParams)
        {
            ...
            // 慣性制約 惯性约束
            // 初期化時のセンターローカル位置を初期化 初始化时的中心本地位置
        ...
        //=========================================================================================
        /// <summary>
        /// チーム更新後処理用作業リスト 小组更新后处理用作业列表
        /// </summary>
        List<ClothProcess> workPostProcessList = new List<ClothProcess>(256);

        /// <summary>
        /// 毎フレーム常に実行するチーム更新 每帧经常执行的团队更新每帧始终执行的团队更新
        /// - 時間の更新と実行回数の算出 时间更新和运行次数计算
        /// </summary>
        internal void AlwaysTeamUpdate()
        {
            ...
            // ジョブでは実行できないチーム更新 无法在作业中运行的团队更新
        ...
        struct CalcCenterAndInertiaAndWindJob : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                {
                    tdata.negativeScaleSign = -1; // マイナススケール時は(-1) 负比例时为（-1）
                    tdata.negativeScaleQuaternionValue = new float4(-math.sign(componentWorldScl), 1); // 回転反転用 旋转反转用
                    tdata.negativeScaleTriangleSign.x = (componentWorldScl.x < 0 || componentWorldScl.z < 0) ? -1 : 1; // TriangleBendingの法線フリップフラグ TriangleBending法线翻转标志
                    tdata.negativeScaleTriangleSign.y = componentWorldScl.x < 0 ? -1 : 1; // TriangleBendingの接線フリップフラグ TriangleBending切线翻转标志
                ...
                {
                    // 軸反転テレポート 轴反转电话端口
                    // 本体がスケール反転するためシミュレーションに影響が出ないように必要な座標系を同様に反転させる 为了避免主体发生缩放反转而对模拟产生影响，同样使必要的坐标系反转
                    // 一旦クロスローカル空間に戻し軸反転してからワールドに書き戻す 暂时返回交叉局部空间轴反转后再写入世界
                ...
                {
                    // センター反転用変換マトリックスを計算(2) 计算中心翻转变换矩阵（2）
        ...
        struct SimulationStepTeamUpdateJob : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                // 今回のフレーム割合を計算する 计算此次的帧比例
                // frameStartTimeからtime区間でのnowUpdateTimeの割合 从frameStartTime到time区间的nowUpdateTime的比例
                ...
                float localMovementSpeed = math.length(localVector) / simulationDeltaTime; // ローカル移動速度(m/s)  局部移动速度（m/s）
                ...
                float localAngleSpeed = math.degrees(localAngle / simulationDeltaTime); // ローカル回転速度(deg/s)  局部旋转速度
                ...
                // ■遠心力用パラメータ算出 离心力用参数计算
                // 今回ステップでの回転速度と回転軸  这次步骤中旋转速度和旋转轴
                cdata.angularVelocity = stepAngle / simulationDeltaTime; // 回転速度(rad/s) 旋转速度
                ...
                if (math.lengthsq(param.worldGravityDirection) > Define.System.Epsilon)
                {
                    ...
                    initLocalGravityDirection.y *= tdata.negativeScaleDirection.y; // Yマイナススケール時のみY軸を反転 仅在Y减比例时反转Y轴
        ...
        //=========================================================================================
        /// <summary>
        /// ここに登録されるのはClothコンポーネントがDisable時に初期化されたプロセス 这里注册的是Cloth组件在禁用时初始化的过程
        /// これらのプロセスはマネージャ側で消滅が監視される 这些过程在管理者方面被监视消失
        /// </summary>
