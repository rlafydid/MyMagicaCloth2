...
namespace MagicaCloth2
{
    ...
    public class VirtualMeshManager : IManager, IValid
    {
        ...
        /// <summary>
        /// 頂点ごとの接続トライアングルインデックスと法線接線フリップフラグ（最大７つ） 每个顶点的连接三角索引和法线切线翻转标志（最多7个）
        /// これは法線を再計算するために用いられるもので７つあれば十分であると判断したもの。 这是为了重新计算法线而使用的东西，如果有7个就足够了。
        /// そのため正確なトライアングル接続を表していない。 因此没有表示正确的三角连接。
        /// データは12-20bitのuintでパックされている 数据以12-20bit的uint进行包装 12（hi）=法线切线的翻转标志（法线：0x1，切线：0x2）。ON时翻转。
        /// 12(hi) = 法線接線のフリップフラグ(法線:0x1,接線:0x2)。ONの場合フリップ。 20（low）=三角索引。
        ...
        /// <summary>
        /// 各頂点の角度計算用のローカル回転 局部旋转以计算每个顶点的角度
        /// pitch/yaw個別制限はv1.0では実装しないので一旦停止させる pitch/yaw个别限制在v1.0中不安装，所以暂时停止
        /// </summary>
        //public ExNativeArray<quaternion> vertexAngleCalcLocalRotations;

        /// <summary>
        /// UV
        /// VirtualMeshのUVはTangent計算用でありテクスチャマッピング用ではないので注意！ 注意，虚拟网格的UV是用于Tangent计算的，而不是用于纹理映射的！
        /// </summary>
        ...
        /// <summary>
        /// エッジ固有フラグ(VirtualMesh.EdgeFlag_~) 虚拟网格.EdgeFlag_~
        /// </summary>
        ...
        public void ExitProxyMesh(int teamId)
        {
            ...
            // 同時に連動するマッピングメッシュも解放する 同时也释放联动映射网格
        ...
        struct CreateProxyMeshUpdateVertexList : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                // BoneClothもMeshClothもすべてスキニングとして登録 也全部注册为蒙皮
        ...
        /// <summary>
        /// プロキシメッシュの頂点スキニングを行い座標・法線・接線を求める 执行代理网格顶点蒙皮以获得坐标法线切线
        /// [BoneCloth][MeshCloth]兼用
        /// 姿勢はワールド座標で格納される 姿势存储在世界坐标中
        /// </summary>
        [BurstCompile]
        struct CalcProxyMeshSkinningJob : IJobParallelForDefer
        {
            ...
            public void Execute(int index)
            {
                ...
                // バインドポーズにスケールが入るので単位化する必要がある 绑定姿势有比例，需要单位化
#if MC2_DEBUG
        ...
        //=========================================================================================
        /// <summary>
        /// クロスシミュレーションの結果をProxyMeshへ反映させる  使ProxyMesh反映交叉模拟的结果
        /// </summary>
        /// <param name="jobHandle"></param>
        /// <returns></returns>
        internal unsafe JobHandle PostProxyMeshUpdate(JobHandle jobHandle)
        {
            ...
            // 今回更新が必要な各インデックスリストを作成する  创建需要此次更新的各索引列表
            ...
            // ラインがある場合はベースラインごとに姿勢を整える  有线的情况下，按基线调整姿势
            if (BaseLineCount > 0)
            {
                // ラインの法線・接線を求める 求出线的法线和切线
                // （ワールド座標空間） （世界坐标空间）
            ...
            // トライアングルがある場合はトライアングル接続情報から最終的な姿勢を求める 在存在三角的情况下，根据三角连接信息求出最终的姿势
            if (TriangleCount > 0)
            {
                // トライアングルの法線・接線を求める 求三角的法线和切线
                // （ワールド座標空間）（世界坐标空间）
                ...
                // トライアングルの法線接線から頂点法線接線を平均化して求める 从三角法线切线平均顶点法线切线
                // （ワールド座標空間）（世界坐标空间）
            ...
            // Transformパーティクルの場合はvertexToTransform回転を乗算してTransformDataに情報を書き戻す Transform对于粒子，通过乘以vertexToTransform旋转将信息写回TransformData
            ...
            // Transformパーティクルは親からのローカル姿勢を計算してTransformData情報に書き込む Transform粒子计算父代的局部姿态并将其写入到变换数据信息中
