    ...
    public class TetherConstraint : IDisposable
    {
        ...
        struct TethreConstraintJob : IJobParallelForDefer
        {
            ...
            public void Execute(int index)
            {
                ...
                // 距離がほぼ０ならば処理をスキップする（エラーの回避） 距离接近零时跳过处理（避免错误）
                if (distance < Define.System.Epsilon)
                    return;

                // 復元距離
                // フラグにより初期姿勢かアニメーション後姿勢かを切り替える 根据标志切换使用初始姿势还是动画后姿势
                ...
                {
                    // 縮んでいる場合は戻りを比較的緩やかにする。これは振動の防止につながる。  收缩时使用较平缓的恢复，防止振动
                ...
                {
                    // 伸びている場合は戻りを急速に行う。その代わり速度影響は弱くする。 拉伸时快速恢复，同时减弱速度影响
