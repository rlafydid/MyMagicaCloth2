    ...
    public static class JobUtility
    {
        ...
        struct CalcUVJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                float2 uv = new float2(v, u); // こちらは縦方向ベースになる 这是纵向基础
                ...
                // 为了避免在方向向量上生成相同的UV，在UV上加上距离错开
                float add = index * 0.0001234f; // UVを微妙にずらすための加算値 用于微妙地错开UV的相加值

                // 頂点間のUVの間隔を広めに取り、同じUVが発生しないようにインデックスを使い微妙に値をずらす
                // 扩大顶点间的UV间隔，为了不发生相同的UV，使用索引微妙地错开值
                uv = uv * 10.0f + add;
#endif
